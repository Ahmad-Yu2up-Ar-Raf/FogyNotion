  ...
  return (
    <TextClassContext.Provider
      value={cn(
        ...
        value === props.value && '  font-poppins_semibold text-secondary dark:text-secondary'
      )}>
      <TabsPrimitive.Trigger
        className={cn(
          'flex h-[calc(100%-1px)] flex-row items-center justify-center gap-1.5 rounded-md border border-transparent px-2 py-0.5 shadow-none shadow-black/5',
          Platform.select({
          ...
          props.value === value && 'bg-secondary/10',
          className
        )}
        {...props}
      />
