...
interface UseScrollAnimationReturn {
  ...
  scrollAnimatedPosition: Animated.SharedValue<number>;
...
export function useScrollAnimation({
  ...
  // ✅ Animated scroll handler — runsOnJS=false untuk performance
  // Dijalankan di UI thread, tidak transform ke JS thread
  const scrollHandler = useAnimatedScrollHandler({
    onScroll: (event) => {
      scrollAnimatedPosition.value = event.contentOffset.y;

      // ✅ Optional callback untuk non-animated logic
      // Gunakan runOnJS jika butuh update state/logging
      if (onScroll) {
        // Uncomment jika butuh JS thread callback
